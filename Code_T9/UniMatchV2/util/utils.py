...
def save_predictions_as_masks(pred, ids, save_dir="predictions"):
    """
    Save predicted segmentation masks as PNG images.

    Args:
        pred (torch.Tensor or np.ndarray): Predicted segmentation masks (N, H, W).
        ids (list of str): Unique IDs corresponding to each mask.
        save_dir (str): Directory where masks will be saved.

    Example:
        pred.shape = (batch_size, height, width)
        ids = ["image1", "image2", ...]
    """
    ...
    # Convert predictions to NumPy array if they are tensors
    if isinstance(pred, torch.Tensor):
        pred = pred.cpu().numpy()  # Move to CPU and convert to numpy

    for i, idx in enumerate(ids):
        mask = pred[i].astype(np.uint8)  # Convert to uint8 format
