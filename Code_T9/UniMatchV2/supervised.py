...
def setup_distributed_training():
    if torch.cuda.device_count() == 1:  # Single GPU (Colab or local machine)
        ...
        return setup_distributed(port=args.port)
...
def evaluate(model, loader, mode, cfg, multiplier=None):
        ...
        for img, mask, id in loader:
            ...
            if mode == 'sliding_window':
                ...
                if multiplier is not None:
                    ...
                    new_h, new_w = (512, 512) if multiplier == 512 else (
            ...
            intersection, union, target = intersectionAndUnion(pred, mask.numpy(), cfg['nclass'], 255)

            dice_class = ((2. * intersection) / (union + target + 1e-10)).astype(np.float32) * 100.0

            reduced_intersection = torch.from_numpy(intersection).cuda()
            reduced_union = torch.from_numpy(union).cuda()
            reduced_target = torch.from_numpy(target).cuda()
            reduced_dice = torch.from_numpy(dice_class).float().cuda()
    ...
    mean_dice = np.mean(reduced_dice.cpu().numpy()[1:])  # Ignore background (class 0)

    return mIoU, iou_class, mean_dice, reduced_dice.cpu().numpy()
...
def main():
    ...
    state_dict = torch.load('/content/drive/MyDrive/MAIA_Work/Thesis/Challenges/FUGC/UniMatch-V2-main/pretrained/dinov2_vits14_pretrain.pth')
    ...
    # Use DistributedSampler only if multiple GPUs
    if world_size > 1:
    ...
    valloader = DataLoader(
        valset, batch_size=1, pin_memory=True, num_workers=1, drop_last=False, sampler=valsampler if valsampler else None
    )
