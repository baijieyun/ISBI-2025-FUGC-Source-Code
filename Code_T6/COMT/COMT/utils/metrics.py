...
class HD(nn.Module):
    ...
    def evaluation(self, pred: sitk.Image, label: sitk.Image):
        ...
        return (result['hd_all'] + result['hd_lower'] + result['hd_upper']) / 3, result
    ...
    def forward(self, pred, label):
        ...
        result_tol, result = self.evaluation(pre_image, truth_image)

        return result_tol, result


    # def cal_hd(self, a, b):
    #     # todo 注意这里有类别不均衡的问题，某类可能没有预测出非零像素点！
    ...
    def cal_hd(self, a, b):
        """
        计算Hausdorff距离，处理类别不均衡问题：
        - 当 a 或 b 中有且只有一个为全零时，返回 373.13。
        - 当 a 和 b 都为全零时，返回 0。
        https://blog.csdn.net/hpulittle_804/article/details/118367573
        """
        # todo 注意这里有类别不均衡的问题，某类可能没有预测出非零像素点！focal loss!
        # 转换为UInt8类型
        a = sitk.Cast(sitk.RescaleIntensity(a), sitk.sitkUInt8)
        b = sitk.Cast(sitk.RescaleIntensity(b), sitk.sitkUInt8)
        
        # 检查非零像素的数量
        non_zero_a = sitk.GetArrayViewFromImage(a).sum() > 0
        non_zero_b = sitk.GetArrayViewFromImage(b).sum() > 0
        
        if not non_zero_a and not non_zero_b:
            # 两个都为全零
            return 0
        elif (not non_zero_a and non_zero_b) or (non_zero_a and not non_zero_b):
            # 仅一个包含非零像素
            return 373.13
        else:
            # 正常计算Hausdorff距离
