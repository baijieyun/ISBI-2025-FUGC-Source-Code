...
class JointTransform2D:
    ...
    def __init__(self, img_size=(544,336), low_img_size=256, ori_size=512, crop=(32, 32), p_flip=0.0, p_rota=0.0, p_scale=0.0,
    ...
    def __call__(self, sample):
        ...
        if np.random.rand() < self.p_gaussn:
            ...
            noise = np.random.normal(loc=0, scale=1, size=self.img_size) * ns
            noise = noise.astype(int)
            noise = np.repeat(np.expand_dims(noise, axis=-1), 3, axis=-1)
