...
class AttU_Net(nn.Module):
    ...
    def forward(self, x,comp_drop=False):
        ...
        if comp_drop:
            for feat in feature:
                bs, dim = feat[0].shape[0], feat[0].shape[-1]
                ...
                feat = feat * dropout_mask.unsqueeze(1)
...
if __name__ == '__main__':
    x = torch.randn(1, 3, 544, 336).cuda()
    model = AttU_Net().cuda()
    out ,out_fp= model(torch.cat((x,x)),comp_drop=True).chunk(2)
    print(out.shape)
